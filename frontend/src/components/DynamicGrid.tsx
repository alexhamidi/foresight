...
export default function DynamicGrid() {
  ...
  const animate = () => {
    ...
    for (const node of nodesRef.current) {
      ...
      const targetX = distance === 0
        ? node.baseX
        : node.baseX - (dx / distance) * force * 20

      const targetY = distance === 0
        ? node.baseY
        : node.baseY - (dy / distance) * force * 20
    ...
    for (const line of linesRef.current) {
      ...
      if (distance < maxDistance) {
        ...
        line.element.setAttribute("stroke", `rgba(0, 0, 0, ${opacity * 0.2 * (opacitySetting / 100)})`)
      } else {
